...
/**
 * Pull the tailored questions generated by the agent pipeline (see
 * src/app/dashboard/student/prep/page.tsx and /api/agents/prep) out of
 * sessionStorage, if the student went through that flow. Otherwise fall back
 * to the original static question bank + random 3, exactly like before --
 * this page still works even if a student skips straight here from the menu.
 */
function loadActiveQuestions() {
    try {
        const stored = sessionStorage.getItem("tailored_prep");
        if (stored) {
            const parsed = JSON.parse(stored);
            if (Array.isArray(parsed.questions) && parsed.questions.length > 0) {
                return { questions: parsed.questions, isTailored: true };
            }
        }
    } catch (err) {
        console.error("Could not read tailored_prep from sessionStorage:", err);
    }

    const indexes = Array.from({ length: questionData.length }, (_, i) => i)
        .sort(() => Math.random() - 0.5)
        .slice(0, 3);
    return { questions: indexes.map((i) => questionData[i]), isTailored: false };
}
...
export default function MockInterviewPage() {
    ...
    useEffect(() => {
        const { questions: activeQuestions, isTailored: tailored } = loadActiveQuestions();
        setQuestions(activeQuestions);
        setIsTailored(tailored);
        // qIdxs is now just an identity index into `questions` -- kept as a
        // separate piece of state so the rest of the component (which was
        // written against "questionData[qIdxs[currIdx]]") barely has to change.
        setQIdxs(activeQuestions.map((_, i) => i));
        // Keep the exact question set used in this session available to the
        // results page, so it renders the same tailored text (not the static bank).
        sessionStorage.setItem("active_questions", JSON.stringify(activeQuestions));
    }, []);
    ...
    useEffect(() => {
        ...
        if (countdown === 0) {
            setIsRecording(true);
            setIsPrep(false);
            setTimeLeft(questions[currIdx][1]);
            startAudioRecording();
        } else {
    ...
    const handleNextQuestion = () => {
        ...
        if (currIdx === qIdxs.length - 1) {
            ...
            setTimeLeft(questions[qIdxs[new_idx]][1])
    ...
    return (
        <div className="h-[90%] w-full flex flex-col items-center justify-center gap-3">
            {
                !isLoading && isTailored &&
                <p className="text-sm text-green-700 bg-green-50 border border-green-200 rounded-lg px-4 py-2">
                    These questions were tailored to the resume + job description you provided.
                </p>
            }
            {
                isLoading ?
                            ...
                            <>
                                {
                                    showQuestion ?
                                        <>
                                            ...
                                            <p className="text-gray-600">{questions[qIdxs[currIdx]][0]}</p>
                                        </> :
                            ...
                            <div className="flex flex-col gap-4 w-full">
                                {
                                    ...
                                    <LinearProgressWithLabel value={timeLeft} variant="determinate"
                                                             total={questions[qIdxs[currIdx]][1]}/>
                                }
                                <button
